...
export const { bind: FunctionProtoBind, toString: FunctionProtoToString } = Function.prototype;

export function createUnmaskableTraps(func: Function) {
    let handshakeUnmaskFlag = false;
    return {
        defineProperty(target: ProxyTarget, key: PropertyKey, desc: PropertyDescriptor) {
            ...
            if (key === LOCKER_NEAR_MEMBRANE_UNMASKED_VALUE_SYMBOL) {
                throw new TypeErrorCtor(ERR_ILLEGAL_PROPERTY_ACCESS);
            }
            return ReflectDefineProperty(target, key, desc);
        },
        get(target: ProxyTarget, key: PropertyKey, receiver?: unknown, handshake = false) {
            // Only allow accessing handshake property values if the "has"
            // trap has been triggered immediately BEFORE and the property
            // does NOT exist.
            handshakeUnmaskFlag &&= handshake;
            const isUnmaskedSymbol = key === LOCKER_NEAR_MEMBRANE_UNMASKED_VALUE_SYMBOL;
            // istanbul ignore else: it is unnecessary to cover the else path here
            if (handshakeUnmaskFlag) {
            ...
            // istanbul ignore else: it is unnecessary to cover the else path here
            if (result !== undefined && isUnmaskedSymbol) {
                throw new TypeErrorCtor(ERR_ILLEGAL_PROPERTY_ACCESS);
            }
            return result;
        },
        getOwnPropertyDescriptor(target: ProxyTarget, key: PropertyKey) {
            ...
            // istanbul ignore else: it is unnecessary to cover the else path here
            if (result && key === LOCKER_NEAR_MEMBRANE_UNMASKED_VALUE_SYMBOL) {
                throw new TypeErrorCtor(ERR_ILLEGAL_PROPERTY_ACCESS);
            }
            return result;
        },
        has(target: ProxyTarget, key: PropertyKey) {
            ...
            if (result) {
                ...
                // [[Prototype]].
                handshakeUnmaskFlag = isUnmaskedSymbol;
            }
            return result;
        },
...
export function getUnmaskedFunction<T extends Function>(func: T): T {
    const unmasked =
        LOCKER_NEAR_MEMBRANE_UNMASKED_VALUE_SYMBOL in func
            ? /* istanbul ignore next */ undefined
            : (func as any)[LOCKER_NEAR_MEMBRANE_UNMASKED_VALUE_SYMBOL];
    return unmasked === undefined ? func : unmasked;
}
...
export function isMaskedFunction(value: any): boolean {
    ...
    return (
        ...
        !(LOCKER_NEAR_MEMBRANE_IS_MASKED_SYMBOL in value) &&
        (value as any)[LOCKER_NEAR_MEMBRANE_IS_MASKED_SYMBOL] === true
    );
}

export function maskFunction<T extends Function>(
    func: Function,
    maskFunc: T,
    ...
    const proxy = new ProxyCtor(maskFunc, {
        ...
        defineProperty(target: ProxyTarget, key: PropertyKey, desc: PropertyDescriptor) {
            ...
            // Defining forgeries of handshake properties is not allowed.
            if (key === LOCKER_NEAR_MEMBRANE_IS_MASKED_SYMBOL) {
                throw new TypeErrorCtor(ERR_ILLEGAL_PROPERTY_ACCESS);
            }
        ...
        get(target: ProxyTarget, key: PropertyKey, receiver: any) {
            ...
            const isProxyMaskedSymbol = key === LOCKER_NEAR_MEMBRANE_IS_MASKED_SYMBOL;
        ...
        getOwnPropertyDescriptor(target: ProxyTarget, key: PropertyKey) {
            ...
            // Getting forged descriptors of handshake properties is not allowed.
            if (result && key === LOCKER_NEAR_MEMBRANE_IS_MASKED_SYMBOL) {
                throw new TypeErrorCtor(ERR_ILLEGAL_PROPERTY_ACCESS);
            }
            return result;
        },
