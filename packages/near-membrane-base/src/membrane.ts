...
export function init(
    ...
    return function curryInit(
                ...
                return <T>function foreignErrorControlFn(...args: any[]): any {
                    try {
                        return function curryFn() {
                            // @ts-ignore
                            const activity: Activity = instrumentation.activityBeacon(
                                'cross-membrane-activity'
                            );
                            activity.start();
                            foreignFn(...args);
                            activity.stop();
        ...
        class BoundaryProxyHandler implements ProxyHandler<ShadowTarget> {
            ...
            // construct trap is generic, and should never change independently of
             ...
             *   const foreignValueOrCallable = foreignCallableGet(
            ...
            private static dynamicGetTrap(
                ...
                while (O !== null) {
                    if (ObjectProtoHasOwnProperty.call(O, key)) {
                        ...
                        const { get: getter, value: localValue } = getOwnPropertyDescriptor(
                            O,
                            key
                        )!;
                        if (getter) {
                            // even though the getter function exists, we can't use Reflect.get
                            // because there might be a distortion for that getter function,
                            // in which case we must resolve the local getter and call it instead.
                ...
                while (O !== null) {
                    if (ObjectProtoHasOwnProperty.call(O, key)) {
                        ...
                        if (setter) {
                            ...
                            // because there might be a distortion for that setter function,
                            // in which case we must resolve the local setter and call it instead.
        ...
        function createLazyDescriptor(
            ...
            // TODO: somehow we need to track the unforgeable/key value pairs in case
            // the local realm ever attempt to access the descriptor, in which case
            // the same mechanism must be applied
            return {
