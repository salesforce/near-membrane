...
EXPECT_PATCHING_SCOPE: {
    ...
    globalThis.expect.assertions = (count) => {
        ...
        if (!Number.isSafeInteger(num)) {
            throw new Error('expect.assertions(count) expects count to be a number >= 0 as the first argument.');
        }
        expectedExpects = num;
    };
    ...
    const checkExpectCount = () => {
        if (expectedExpects !== null && actualExpects !== expectedExpects) {
            throw new Error(`Expected ${expectedExpects} expect${expectedExpects !== 1 ? 's' : ''} to be called, ${actualExpects} expect${actualExpects !== 1 ? 's were' : ' was'} actually called.`);
        }
    };
